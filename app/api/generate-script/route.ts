...
export async function POST(req: Request) {
  ...
  userBlocks.push({
    ...
    text:
      ...
      `=== 숏츠 전문가 원칙 (반드시 적용) ===\n` +
      `1. HOOK — 첫 3초가 시청 유지율 50~60%를 결정합니다 (Opus.pro 데이터).\n` +
      `   - 12자 이내. 다음 5가지 패턴 중 하나만 골라 적용:\n` +
      `     · 질문형: "이 가격 진짜예요?", "사과 1.5kg에 9,900원?"\n` +
      `     · 숫자/충격: "${storeName} 이번주 1+1", "한우 30% 할인"\n` +
      `     · 시간 압박: "오늘까지만!", "주말 한정 타임특가"\n` +
      `     · 비밀/팁: "마트 단골만 아는 시간특가"\n` +
      `     · 문제 제기: "장보기 비싸졌죠?", "이거 모르면 손해"\n` +
      `   - "안녕하세요", "오늘은~", "여러분~" 같은 평범한 인사 절대 금지.\n` +
      `2. SEGMENTS — 코너별 핵심 한 가지.\n` +
      `   - 한 코너 = 한 메시지. 군더더기 없이 짧은 문장 2~3개.\n` +
      `   - 코너당 약 ${perCornerChars}자.\n` +
      `   - 마트 광고 표준 어휘를 적극 활용 (가능할 때):\n` +
      `     "시간특가", "타임세일", "오늘만", "이번주", "주말 한정", "한정수량",\n` +
      `     "1+1", "2+1", "묶음할인", "한팩 ~~원", "kg당 ~원", "조기품절 주의"\n` +
      `   - 가격/할인율은 사진이나 힌트에 명확히 보일 때만 정확히 인용 (없는 숫자 절대 지어내지 말 것).\n` +
      `   - highlight 필드: 그 코너에서 화면 강조할 단어/숫자 1개 (예: "9,900원", "30%", "1+1"). 없으면 빈 문자열.\n` +
      `3. CTA — 마지막 콜투액션.\n` +
      `   - 12자 이내. 행동 동사 또는 시간 압박으로 끝내기.\n` +
      `   - 예시: "지금 ${storeName}으로!", "이번주 일요일까지!", "오늘 안에 오세요", "물량 떨어지기 전에"\n` +
      `4. 전체 톤 — 친근하고 빠른 구어체. 한 문장 15자 내외. 과장 감탄사("와~", "어머") 자제.\n` +
      `5. 페이싱 — 자막이 1.5~2초마다 바뀌도록 짧은 문장으로 끊기 (한 호흡 ≤ 12자).\n` +
      `6. 무음 금지 — TTS가 자연스럽게 읽도록 마침표/쉼표 정확히.\n\n` +
      `=== 출력 형식 ===\n` +
