...
export async function POST(req: Request) {
  ...
  userBlocks.push({
    ...
    text:
      ...
      `   - **🚫 환각 절대 금지** — 사진에 명확히 보이지 않는 상품/문구는 절대 언급하지 않습니다. ` +
      `상품 종류가 확실하지 않으면 두루뭉술하게 ("신선한 식재료", "오늘 입고된 상품" 등) 처리. ` +
      `사진에서 확인 안 되는 디테일(원산지, 특정 부위, 행사명 등)을 추측해서 말하지 마세요.\n` +
      `   - 텍스트 힌트가 있으면 그게 정답. 힌트와 사진이 다르면 힌트 우선.\n` +
  ...
  try {
    const message = await client.messages.create({
      ...
      system:
        ...
        '**환각 절대 금지** — 사진에 명확히 보이지 않는 상품, 문구, 디테일은 추측해서 쓰지 않습니다. 확실하지 않으면 두루뭉술한 표현으로 처리합니다. 사용자가 제공한 텍스트 힌트가 항상 우선입니다. ' +
